                        ...
                        Rectangle {
                            ...
                            ColumnLayout {
                                ...
                                GridLayout {
                                    ...
                                    // Speed: dual slider + track
                                    RowLayout {
                                        Layout.columnSpan: 2
                                        Layout.fillWidth: true
                                        spacing: 8
                                        Slider {
                                            Layout.fillWidth: true; from: 1; to: 400
                                            value: { var _ = root._audioKfRev; return Number(root.audioEvaluatedParam("speed", 100)); }
                                            onMoved: {
                                                ...
                                                if (model && idx >= 0 && Workspace.currentTimeline) {
                                                    ...
                                                    if (rawKfs.length > 0) Workspace.currentTimeline.setKeyframe(targetClipId, idx, "speed", root._audioRelFrame, value, {"interp": "linear"});
                                                    else root.setAudioParam("speed", value);
                                                } else root.setAudioParam("speed", value);
                                            }
                                        }
                                        TextField {
                                            Layout.preferredWidth: 64
                                            text: { var _ = root._audioKfRev; return Number(root.audioEvaluatedParam("speed", 100)).toFixed(0); }
                                            horizontalAlignment: Text.AlignHCenter
                                            onEditingFinished: root.setAudioParam("speed", Number(text))
                                        }
                                        Button {
                                            ...
                                            text: {
                                                ...
                                                return qsTr("Speed") + (rawKfs.length > 0 ? (il[root._audioInterpAt("speed", root._audioRelFrame)] || "") : "");
                                            }
                                            onClicked: {
                                                var model = root.audioEffectModel(); var idx = root.audioEffectIndex();
                                                if (!model || idx < 0 || !Workspace.currentTimeline) return;
                                                if (model.keyframeListForUi("speed").length === 0) {
                                                    Workspace.currentTimeline.setKeyframe(targetClipId, idx, "speed", 0, root.audioParamValue("speed", 100), {"interp": "linear"});
                                                    Workspace.currentTimeline.setKeyframe(targetClipId, idx, "speed", root._audioClipDur, root.audioParamValue("speed", 100), {"interp": "linear"});
                                                }
                                                var win = WindowManager.getWindow("easingConfig");
                                                if (win) win.openConfig({"clipId": targetClipId, "effectIndex": idx, "effectModel": model, "paramName": "speed", "keyframeFrame": root._audioRelFrame});
                                            }
                                        }
                                        TextField {
                                            Layout.preferredWidth: 64
                                            ...
                                            horizontalAlignment: Text.AlignHCenter
                                            enabled: root.audioKeyframeList("speed").length >= 2
                                    ...
                                    // Direct: dual slider + track
                                    RowLayout {
                                        Layout.columnSpan: 2
                                        Layout.fillWidth: true
                                        spacing: 8
                                        Slider {
                                            Layout.fillWidth: true; from: 0; to: 60
                                            value: { var _ = root._audioKfRev; return Number(root.audioEvaluatedParam("directTime", 0)); }
                                            onMoved: {
                                                var model = root.audioEffectModel(); var idx = root.audioEffectIndex();
                                                if (model && idx >= 0 && Workspace.currentTimeline) {
                                                    var rawKfs = model.keyframeListForUi("directTime");
                                                    if (rawKfs.length > 0) Workspace.currentTimeline.setKeyframe(targetClipId, idx, "directTime", root._audioRelFrame, value, {"interp": "linear"});
                                                    else root.setAudioParam("directTime", value);
                                                } else root.setAudioParam("directTime", value);
                                            }
                                        }
                                        TextField {
                                            Layout.preferredWidth: 64
                                            text: { var _ = root._audioKfRev; return Number(root.audioEvaluatedParam("directTime", 0)).toFixed(2); }
                                            horizontalAlignment: Text.AlignHCenter
                                        ...
                                        Button {
                                            ...
                                            text: {
                                                ...
                                                return qsTr("Direct") + (rawKfs.length > 0 ? (il[root._audioInterpAt("directTime", root._audioRelFrame)] || "") : "");
                                            }
                                            onClicked: {
                                                var model = root.audioEffectModel(); var idx = root.audioEffectIndex();
                                                if (!model || idx < 0 || !Workspace.currentTimeline) return;
                                                if (model.keyframeListForUi("directTime").length === 0) {
                                                    Workspace.currentTimeline.setKeyframe(targetClipId, idx, "directTime", 0, root.audioParamValue("directTime", 0), {"interp": "linear"});
                                                    Workspace.currentTimeline.setKeyframe(targetClipId, idx, "directTime", root._audioClipDur, root.audioParamValue("directTime", 0), {"interp": "linear"});
                                                }
                                                var win = WindowManager.getWindow("easingConfig");
                                                if (win) win.openConfig({"clipId": targetClipId, "effectIndex": idx, "effectModel": model, "paramName": "directTime", "keyframeFrame": root._audioRelFrame});
                                            }
                                        }
                                        TextField {
                                            Layout.preferredWidth: 64
                                            ...
                                            horizontalAlignment: Text.AlignHCenter
                                            enabled: root.audioKeyframeList("directTime").length >= 2
                                            opacity: enabled ? 1 : 0.45
                                            onEditingFinished: {
                                                var model = root.audioEffectModel(); var idx = root.audioEffectIndex();
                                                if (!model || idx < 0) return;
