...
private slots:
    ...
    void resolveAudioTime_directMode() {
        QCOMPARE(resolveAudioTime(0.0, true, 5.0, 0.0, AviQtl::kDefaultSpeed), 5.0);
        QCOMPARE(resolveAudioTime(10.0, true, 3.5, 1.0, 200.0), 3.5);
    }

    void resolveAudioTime_normalMode() {
        // (relTime * (speed / AviQtl::kDefaultSpeed)) + startTime
        QCOMPARE(resolveAudioTime(0.0, false, 0.0, 1.0, AviQtl::kDefaultSpeed), 1.0);
        QCOMPARE(resolveAudioTime(2.0, false, 0.0, 0.0, AviQtl::kDefaultSpeed), 2.0);
    ...
    void resolveVideoTime_directMode() {
        ...
        QCOMPARE(resolveVideoTime(0, 30.0, true, 60.0, 0.0, AviQtl::kDefaultSpeed), 2.0);
        QCOMPARE(resolveVideoTime(0, 24.0, true, 48.0, 0.0, AviQtl::kDefaultSpeed), 2.0);
    }

    void resolveVideoTime_normalMode() {
        // startSec + (relTime * (speed / AviQtl::kDefaultSpeed))
        // relFrame=30, sourceFps=30, startFrame=0, speed=100 -> 0 + (30/30 * 1.0) = 1.0
        QCOMPARE(resolveVideoTime(30, 30.0, false, 0.0, 0.0, AviQtl::kDefaultSpeed), 1.0);
        // relFrame=30, sourceFps=30, startFrame=60, speed=100 -> 2.0 + 1.0 = 3.0
        QCOMPARE(resolveVideoTime(30, 30.0, false, 0.0, 60.0, AviQtl::kDefaultSpeed), 3.0);
    ...
    void maxVideoDurationFrames_basic() {
        ...
        QCOMPARE(maxVideoDurationFrames(100, 30.0, AviQtl::kDefaultSpeed, 0.0, 30), 100);
    }
    ...
    void maxVideoDurationFrames_withStartFrame() {
        ...
        QCOMPARE(maxVideoDurationFrames(100, 30.0, AviQtl::kDefaultSpeed, 30.0, 30), 70);
    }
    ...
    void maxVideoDurationFrames_startBeyondEnd() {
        ...
        QCOMPARE(maxVideoDurationFrames(100, 30.0, AviQtl::kDefaultSpeed, 200.0, 30), 0);
    }

    void maxVideoDurationFrames_differentProjectFps() {
        // 60 frames at 30fps = 2.0s, projectFps=60 -> 2.0s * 60fps = 120 frames
        QCOMPARE(maxVideoDurationFrames(60, 30.0, AviQtl::kDefaultSpeed, 0.0, 60), 120);
    }
};
