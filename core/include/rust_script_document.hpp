...
namespace AviQtl::RustCore::Script {
...
inline std::optional<QVariantMap> parse(const QString &input) {
    const QByteArray encoded = input.toUtf8();
    const auto *data = reinterpret_cast<const std::uint8_t *>(encoded.constData());
    const auto size = static_cast<std::size_t>(encoded.size());
    std::size_t required = 0;
    auto status = static_cast<Status>(aviqtl_script_metadata_parse_json(data, size, nullptr, 0, &required));
    if (status != Status::BufferTooSmall)
        return std::nullopt;
    std::vector<std::uint8_t> output(required);
    std::size_t written = 0;
    status = static_cast<Status>(aviqtl_script_metadata_parse_json(data, size, output.data(), output.size(), &written));
